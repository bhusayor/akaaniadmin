...
import IngredientFormModal from './IngredientFormModal.jsx';
...
import { USDA_SOURCE } from '../../lib/api.js';
...
function SourceBadge({ record }) {
  ...
  if (record.source === USDA_SOURCE) {
    return <Badge tone="mint" title={`FDC ${record.source_code || ''}`}>USDA</Badge>;
  }
...
/** The in-admin ingredient library: macros, WAFCT matching, CSV import/export. */
export default function LocalLibrary({ tabs }) {
  ...
  const rows = useMemo(() => {
    ...
    return ingredients.filter((r) => {
      ...
      if (sourceFilter === 'usda' && r.source !== USDA_SOURCE) return false;
      if (sourceFilter === 'ai' && !isAiSource(r.source)) return false;
      if (sourceFilter === 'manual' && (r.source === WAFCT_SOURCE || r.source === USDA_SOURCE
        || isAiSource(r.source) || !hasNutrition(r))) return false;
        ...
        left={
          <>
            ...
            <FilterSelect value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value)}>
              ...
              <option value="usda">USDA (measured)</option>
